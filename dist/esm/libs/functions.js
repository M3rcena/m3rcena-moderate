...
export const checkPackageUpdates = async function (name, notifyUpdate) {
    ...
    try {
        ...
        if (stdout.trim().toString() > moderate_package.version) {
            ...
            const msg = chalk(`New ${chalk.green('version')} of ${chalk.yellow('@m3rcena/moderate')} is available!`);
            const msg2 = chalk(`${chalk.red(moderate_package.version)} -> ${chalk.green(stdout.trim().toString())}`);
            const tip = chalk(`Registry: ${chalk.cyan('https://www.npmjs.com/package/@m3rcena/moderate')}`);
            const install = chalk(`Run ${chalk.green(`npm i @m3rcena/moderate@${stdout.trim().toString()}`)} to update!`);
