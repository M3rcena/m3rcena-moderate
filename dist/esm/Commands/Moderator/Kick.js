...
import { checkPackageUpdates } from "../../libs/functions.js";
import { OptionsChecking } from "../../libs/OptionChecking.js";
const Kick = async (options) => {
    // Check type
    OptionsChecking(options, "Kick");
    let interaction = options.interaction;
    if (!interaction)
        throw new Error(chalk.red("[@m3rcena/moderate] Kick Error:") + " No interaction provided.");
    let client = options.client;
    if (!interaction.channel)
        throw new Error(chalk.red("[@m3rcena/weky] Kick Error:") + " No channel found on Interaction.");
    if (!interaction.channel.isSendable())
        throw new Error(chalk.red("[@m3rcena/weky] Kick Error:") + " Channel is not sendable.");
    if (!interaction.guild)
        throw new Error(chalk.red("[@m3rcena/weky] Kick Error:") + " No guild found on Interaction.");
    try {
